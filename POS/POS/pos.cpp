...
pos<_m_particles, _n_dimensions>::pos(const int stop_criteria, const int _w, const int _c1, const int _c2, const int from, const int to) :
	_stop(stop_criteria),
	_w(_w),
	_c1(_c1),
	_c2(_c2)
{
	...
	display_matrix("Initial Velocity matrix", &v[0][0]);
...
template<size_t _m_particles, size_t _n_dimensions>
double pos<_m_particles, _n_dimensions>::uniform_random(const int from, const int to)
{
	return _r_gen.nextRand(from, to); 
}
...
template<size_t _m_particles, size_t _n_dimensions>
void pos<_m_particles, _n_dimensions>::populate_matrix(double* mat, const int from, const int to)
{
	for (int i = 0; i < _m_particles; i++) {
...
template<size_t _m_particles, size_t _n_dimensions>
void pos<_m_particles, _n_dimensions>::display_matrix(const std::string& matrix_name, const double* mat)
{
...
template<size_t _m_particles, size_t _n_dimensions>
void pos<_m_particles, _n_dimensions>::display_vector(const std::string& vector_name, const double* vect)
{
