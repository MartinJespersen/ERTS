...
pos_algo::pos_algo(float g[_n_dimensions]) :
	_stop(100),
	_w(0.80f),	
	_c1(1.98f),
	_c2(1.95f)
...
float pos_algo::uniform_random(const signed short int f, const signed short int t)
{
	return r_gen.nextRand(f, t); 
}

void pos_algo::pos_start(float g[_n_dimensions])
{
	float global_objective_function_res = objective_function(g);
	float local_objective_function_res[_m_particles];
	float x_objective_function_res = 0;
...
float pos_algo::objective_function(float x[_n_dimensions])
{
	return x[0] * x[0] + x[1] * x[1];
}


void pos_algo::populate_matrix(float mat[_m_particles][_n_dimensions], const short int f, const short int t)
{
	for (unsigned short int i = 0; i < _m_particles; i++) {
...
void pos_algo::copy_matrix(float copy_from[_m_particles][_n_dimensions], float copy_to[_m_particles][_n_dimensions])
{
	for (unsigned short int i = 0; i < _m_particles; i++) {
		for (unsigned short int j = 0; j < _n_dimensions; j++) {
...
void pos_algo::copy_vector(float copy_from[_n_dimensions], float copy_to[_n_dimensions])
{
	for (unsigned short int i = 0; i < _n_dimensions; i++) {
...
#ifndef __ON_TARGET__
void pos_algo::display_matrix(const std::string& matrix_name, const float mat[_m_particles][_n_dimensions])
{
...
void pos_algo::display_vector(const std::string& vector_name, const float vect[_n_dimensions])
{
